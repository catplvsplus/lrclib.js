...
<div {...$$props} class={cn("flex flex-col", $$props.class)}>
    ...
    <Tabs.Root value={track.isSynced ? 'synced' : 'plain'} class="w-full h-full">
        <Tabs.List class="grid w-full grid-cols-2 bg-black/15 dark:bg-muted">
            <Tabs.Trigger value="synced" class='data-[state="active"]:text-primary data-[state="active"]:font-bold data-[state="active"]:bg-primary/20' disabled={!track.isSynced}>Synced Lyrics</Tabs.Trigger>
            <Tabs.Trigger value="plain" class='data-[state="active"]:text-primary data-[state="active"]:font-bold data-[state="active"]:bg-primary/20' disabled={!track.isPlain}>Plain Lyrics</Tabs.Trigger>
        </Tabs.List>
        ...
        <Tabs.Content value="plain">
            <Card.Root class="bg-black/15 dark:bg-muted">
                <Card.Content class="p-4 relative">
                    <div class="absolute top-0 right-0 m-2">
                        <CopyButton text={track.syncedLyrics} select={plainLyricsContainer} class="w-full"/>
                    </div>
